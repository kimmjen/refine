...
| 대시보드 | 링크 상세 (AI 요약·태그) |
|---|---|
| ![대시보드](./docs/screenshots/dashboard.png) | ![링크 상세](./docs/screenshots/link-detail.png) |

| AI 설정 | 관리자 대시보드 |
|---|---|
| ![AI 설정](./docs/screenshots/settings.png) | ![관리자](./docs/screenshots/admin.png) |
...
- **사용자별 AI 설정** -- 프로바이더 API 키 (Gemini / OpenAI / Claude), 자동 기능 토글, 출력 언어 설정

> **참고:** AI 분류·요약·태그는 현재 **Google Gemini** (서버 `GEMINI_API_KEY`) 로 실행됩니다. OpenAI (ChatGPT), Anthropic (Claude) API 키는 설정에서 등록할 수 있지만 멀티 프로바이더 실행은 아직 연결되지 않았습니다 -- 로드맵에 있습니다.
...
```
supabase/migrations/000_initial_schema.sql   # 기본 테이블 (신규 설치용)
